...
//        list($MySettings, $MyModuleSettings, $MyModules) = $this->LoadPhpFile($sConfigFile);

        try {
            $sConfigFileYaml = $this->ConfigPathPhpToYaml($sConfigFile);
            $this->CheckFile('configuration', $sConfigFileYaml);
            list($MySettings, $MyModuleSettings, $MyModules) = $this->LoadYamlFile($sConfigFileYaml);
        } catch (ConfigException $configException) {
            //fallback to the php conf file
            $this->CheckFile('configuration', $sConfigFile);
            list($MySettings, $MyModuleSettings, $MyModules) = $this->LoadPhpFile($sConfigFile);
        }
	...
	public function WriteToFile($sFileName = '')
	{
        ...
        $sFileNameYaml = $this->ConfigPathPhpToYaml($sFileName);
        $this->WriteToFileYamlFormat($sFileNameYaml, $aConfigSettings);

        return $this->WriteToFilePhpFormat($sFileName, $aConfigSettings);
    }
	...
	private function LoadYamlFile($sConfigFile)
    {
        $yamlToArray = \Symfony\Component\Yaml\Yaml::parseFile($sConfigFile);

        $MySettings = $yamlToArray['settings'];
        $MyModuleSettings = $yamlToArray['module_settings'];
        $MyModules = $yamlToArray['modules'];

        return array($MySettings, $MyModuleSettings, $MyModules);
    }
     ...
     * @throws ConfigException
     */
    private function LoadPhpFile($sConfigFile)
    {
     ...
     * @return string
     */
    private function ConfigPathPhpToYaml($sConfigFile)
    {
        $dirname = dirname($sConfigFile);
        $basename = basename($sConfigFile);

        return "{$dirname}/{$basename}.yaml";
    }

    private function WriteToFileYamlFormat($sFileName, $aConfigSettings)
    {
        ...
        foreach ($aConfigSettings as $sPropCode => $aSettingInfo) {
            ...
            if ($aSettingInfo['show_in_conf_sample'] || (!empty($aSettingInfo['source_of_value']) && ($aSettingInfo['source_of_value'] != 'unknown'))) {
                ...
                switch ($sType) {
                    case 'bool':
                        $sSeenAs = (bool) $aSettingInfo['value'];
                        break;
                    default:
                ...
                fwrite($hFile, "    ".\Symfony\Component\Yaml\Yaml::dump(array($sPropCode => $sSeenAs))."\n");
            }
        }


        fwrite($hFile, "module_settings: \n");
        fwrite($hFile, "    ".\Symfony\Component\Yaml\Yaml::dump($this->m_aModuleSettings)."\n");
        ...
        fwrite($hFile, "#\n");
        fwrite($hFile, "#\n");
        fwrite($hFile, "modules: \n");
        fwrite($hFile, "    addons: \n");
        fwrite($hFile, "        ".\Symfony\Component\Yaml\Yaml::dump($this->m_aAddons)."\n");
     ...
     * @throws ConfigException
     */
    private function WriteToFilePhpFormat($sFileName, $aConfigSettings)
    {
        $hFile = @fopen($sFileName, 'w');
        if ($hFile !== false) {
            fwrite($hFile, "<?php\n");

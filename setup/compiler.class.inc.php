...
class MFCompiler
{
	...
	protected function DoCompile($sTempTargetDir, $sFinalTargetDir, $oP = null, $bUseSymbolicLinks = false)
	{
		...
		$aAutoloadClassPath = array(); //class to path mapping, will be used to generate a lazy autoloader
		$aClassHierarchy = array();
		...
		foreach($aModules as $foo => $oModule)
		{
			...
			else
			{
				...
				foreach($oClasses as $oClass)
				{
					...
					try
					{
						list($sFilePath, $sParentClass) = $this->CompileClass($oClass, $sTempTargetDir, $sFinalTargetDir, $sRelativeDir);
						$aAutoloadClassPath[$sClass] = "'$sClass' => MODULESROOT . '$sFilePath'";
						$aClassHierarchy[$sClass]['extends'] = $sParentClass;
						$sCompiledCode .= "/**  $sClass initialization code is within $sFilePath */";
					}
...
EOF
		...
		$aClassHierarchy = array_filter($aClassHierarchy, function($aCandidate) use($aAutoloadClassPath) {
			return array_key_exists($aCandidate['extends'], $aAutoloadClassPath);
		});
		...
		$sPHPFileContent .= "\n\$aAutoloadClassPath = array(".implode(",\n", $aAutoloadClassPath).",\n);\n";
		$sPHPFileContent .= "\nksort(\$aAutoloadClassPath);\n";
		$sPHPFileContent .= "MetaModel::SetAutoloadClassPath(\$aAutoloadClassPath);\n";
		$sPHPFileContent .= "MetaModel::SetClassHierarchy(".var_export($aClassHierarchy, true).");\n";
...
EOF
		...
		$sFileFullPath = "{$sTempTargetDir}/{$sFilePath}";
		@mkdir($sTempTargetDir.'/classes');

		file_put_contents($sFileFullPath, "<?php\n /** This file is autogenerated by iTop's compiler */\n\n$sPHP\n");
		if (!file_exists($sFileFullPath))
		{
			throw new Exception('Could not write class file '.$sFileFullPath);
		}

		return array($sFilePath, $sParentClass);
	}

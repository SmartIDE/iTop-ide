			...
			if($oField !== null)
			{
				if ($this->sMode !== static::ENUM_MODE_VIEW)
				{
					...
					if (in_array(get_class($oField), array('Combodo\\iTop\\Form\\Field\\SelectObjectField', 'Combodo\\iTop\\Form\\Field\\LinkedSetField')))
					{
						if ($this->container !== null)
						{
							...
							$sSearchEndpoint = $this->container->get('router')->generate('p_object_search_generic', array(
					...
					if (in_array(get_class($oField), array('Combodo\\iTop\\Form\\Field\\LinkedSetField')))
					{
						if ($this->container !== null)
						{
							$oField->SetInformationEndpoint($this->container->get('router')->generate('p_object_get_informations_json'));
						}
					}
					...
					if (in_array(get_class($oField), array('Combodo\\iTop\\Form\\Field\\SelectObjectField')))
					{
						if ($this->container !== null)
						{
							...
							$oScopeSearch = $this->container->get('scope_validator')->GetScopeFilterForProfiles(UserRights::ListProfiles(), $oScopeOriginal->GetClass(), UR_ACTION_READ);
				...
				if (in_array(get_class($oField), array('Combodo\\iTop\\Form\\Field\\LinkedSetField')))
				{
					...
					if ($this->container !== null)
					{
						...
						$aAttCodesToDisplay = ApplicationHelper::GetLoadedListFromClass($this->container->getParameter('combodo.portal.instance.conf'), $oField->GetTargetClass(), 'list');
		...
		if (class_exists('Attachment') && class_exists('AttachmentPlugIn'))
		{
			...
			if ($bClassAllowed)
			{
				...
				$oField->SetLabel(Dict::S('Portal:Attachments'))
					->SetUploadEndpoint($this->container->get('router')->generate('p_object_attachment_add'))
					->SetDownloadEndpoint($this->container->get('router')->generate('p_object_attachment_download', array('sAttachmentId' => '-sAttachmentId-')))
					->SetTransactionId($oForm->GetTransactionId())
					->SetAllowDelete($this->container->getParameter('combodo.portal.instance.conf')['properties']['attachments']['allow_delete'])
	...
	public function OnSubmit($aArgs = null)
	{
		...
		if ($this->oForm->Validate())
		{
			...
			try
			{
				...
				{
					$sTriggersQuery = $this->container->getParameter('combodo.portal.instance.conf')['properties']['triggers_query'];

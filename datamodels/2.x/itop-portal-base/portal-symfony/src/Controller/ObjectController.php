...
class ObjectController extends AbstractController
{
	...
	public function ViewAction(Request $oRequest, $sObjectClass, $sObjectId)
	{
		...
		if ($sObjectClass === '' || $sObjectId === '')
		{
            ...
            return new Response(Dict::Format('UI:Error:2ParametersMissing', 'class', 'id'), 500);
		}

		// Checking security layers
		if (!SecurityHelper::IsActionAllowed($scopeValidator, $isDebugEnabled, UR_ACTION_READ, $sObjectClass, $sObjectId))
		{
			IssueLog::Warning(__METHOD__ . ' at line ' . __LINE__ . ' : User #' . UserRights::GetUserId() . ' not allowed to read ' . $sObjectClass . '::' . $sObjectId . ' object.');
            return new Response(Dict::S('UI:ObjectDoesNotExist'), 404);
		}

		// Retrieving object
		$oObject = MetaModel::GetObject($sObjectClass, $sObjectId, false /* MustBeFound */, $this->get('scope_validator')->IsAllDataAllowedForScope(UserRights::ListProfiles(), $sObjectClass));
		if ($oObject === null)
		{
		...
		$sOperation = $this->get('request_manipulator')->ReadParam('operation', '');

		$aData = array('sMode' => 'view');
		$aData['form'] = $this->HandleForm($oRequest, $this->container, $aData['sMode'], $sObjectClass, $sObjectId);
		...
		if (SecurityHelper::IsActionAllowed($scopeValidator, $isDebugEnabled, UR_ACTION_MODIFY, $sObjectClass, $sObjectId))
		{
		    $oModifyButton = new URLButtonItem(
		        'modify_object',
                Dict::S('UI:Menu:Modify'),
				$this->get('router')->generate('p_object_edit', array('sObjectClass' => $sObjectClass, 'sObjectId' => $sObjectId))
            );
		...
		else
		{
			...
			$sBrickId = $this->get('request_manipulator')->ReadParam('sBrickId', '');
			if (!empty($sBrickId))
			{
	...
	public function EditAction(Request $oRequest, $sObjectClass, $sObjectId)
	{
		...
		{
			IssueLog::Info(__METHOD__ . ' at line ' . __LINE__ . ' : sObjectClass and sObjectId expected, "' . $sObjectClass . '" and "' . $sObjectId . '" given.');
			throw new HttpException(500, Dict::Format('UI:Error:2ParametersMissing', 'class', 'id'));
		}
		...
		if (!SecurityHelper::IsActionAllowed($scopeValidator, $isDebugEnabled, UR_ACTION_MODIFY, $sObjectClass, $sObjectId) && !$bAllowWrite)
		{
			...
			throw new HttpException(404, Dict::S('UI:ObjectDoesNotExist'));
		}
	...
	public function CreateAction(Request $oRequest, $sObjectClass)
	{
		...
		// Checking security layers
		if (!SecurityHelper::IsActionAllowed($scopeValidator, $isDebugEnabled, UR_ACTION_CREATE, $sObjectClass))
		{
		...
		$aData['form'] = $this->HandleForm($oRequest, $this->container, $aData['sMode'], $sObjectClass);
	...
	public function CreateFromFactoryAction(Request $oRequest, $sObjectClass, $sObjectId, $sEncodedMethodName)
	{
		...
		if (!is_callable($sMethodName))
		{
			...
			throw new HttpException(500, 'Invalid factory method "' . $sMethodName . '" used when creating an object');
		}
...
//		);
//		$sRedirectRoute = $this->get('router')->generate('p_object_create', $aRouteParams);
//		// - Request
...
//		return $oApp->handle($oSubRequest, HttpKernelInterface::SUB_REQUEST, true);
        ...
        $attributes['_controller'] = 'Combodo\\iTop\\Portal\\Controller\\ObjectController::CreateAction';
        $attributes['sObjectClass'] = get_class($oTargetObject);
	...
	{
        $lifecycleValidator = $this->get('lifecycle_validator');
        $combodoPortalInstanceConf = $this->getParameter('combodo.portal.instance.conf');
        /** @var array $combodoPortalInstanceForms */
        $combodoPortalInstanceForms = $this->getParameter('combodo.portal.instance.forms');
		...
		if ($sObjectClass === '' || $sObjectId === '' || $sStimulusCode === '')
		{
			...
			throw new HttpException(500, Dict::Format('UI:Error:3ParametersMissing', 'class', 'id', 'stimulus'));
		}

		// Checking security layers
        if(!SecurityHelper::IsStimulusAllowed($lifecycleValidator, $sStimulusCode, $sObjectClass))
		{
			throw new HttpException(404, Dict::S('UI:ObjectDoesNotExist'));
		}
        ...
        $aStimuliForms = ApplicationHelper::GetLoadedFormFromClass($combodoPortalInstanceForms, $sObjectClass, 'apply_stimulus');
		...
		$aData['form'] = $this->HandleForm($oRequest, $this->container, $aData['sMode'], $sObjectClass, $sObjectId, $aFormProperties);
		$aData['form']['title'] = Dict::Format('Brick:Portal:Object:Form:Stimulus:Title');
		$aData['form']['validation']['redirection'] = array(
			'url' => $this->get('router')->generate('p_object_edit', array('sObjectClass' => $sObjectClass, 'sObjectId' => $sObjectId))
		);
		...
		if (empty($sOperation))
		{
			if (isset($aData['form']['editable_fields_count']) && $aData['form']['editable_fields_count'] === 0)
			{
				...
				$aData = array('sMode' => 'apply_stimulus');
				$aData['form'] = $this->HandleForm($oSubRequest, $this->container, $aData['sMode'], $sObjectClass, $sObjectId, $aFormProperties);
				// Redefining the array to be as simple as possible :
				$aData = array('redirection' =>
					array('url' => $this->get('router')->generate('p_object_edit', array('sObjectClass' => $sObjectClass, 'sObjectId' => $sObjectId)))
				);
			}
		}
     ...
     */
	public static function HandleForm(Request $oRequest, Container $container, $sMode, $sObjectClass, $sObjectId = null, $aFormProperties = null)
	{
        /** @var array $combodoPortalInstanceForms */
	    $combodoPortalInstanceForms = $container->getParameter('combodo.portal.instance.forms');
        /** @var array $combodoPortalInstanceId */
        $combodoPortalInstanceId = $container->getParameter('combodo.portal.instance.id');
        /** @var Router $router */
        $router = $container->get('router');
        /** LifecycleValidatorHelper $lifecycleValidator */
        $lifecycleValidator = $container->get('lifecycle_validator');
        /** @var ScopeValidatorHelper $scopeValidator */
        $scopeValidator = $container->get('scope_validator');
        /** @var ContextManipulatorHelper $contextManipulator */
        $contextManipulator = $container->get('context_manipulator');
        /** @var RequestManipulatorHelper $requestManipulator */
        $requestManipulator = $container->get('request_manipulator');

	    $aFormData = array();
		$sOperation = $requestManipulator->ReadParam('operation', '');
		$bModal = ($oRequest->isXmlHttpRequest() && empty($sOperation));

		// - Retrieve form properties
		if ($aFormProperties === null)
		{
			$aFormProperties = ApplicationHelper::GetLoadedFormFromClass($combodoPortalInstanceForms, $sObjectClass, $sMode);
		}
		...
		if (empty($sOperation))
		{
            ...
            $sActionRulesToken = $requestManipulator->ReadParam('ar_token', '');
			...
			if ($sObjectId === null)
			{
				...
				// Preparing object
				$contextManipulator->PrepareObject($aActionRules, $oObject);
			...
			{
				$oObject = MetaModel::GetObject($sObjectClass, $sObjectId, true, $scopeValidator->IsAllDataAllowedForScope(UserRights::ListProfiles(), $sObjectClass));
			}
			...
			if ($sMode !== 'apply_stimulus')
			{
				...
				{
					if(SecurityHelper::IsStimulusAllowed($lifecycleValidator, $sStimulusCode, $sObjectClass, $oSetToCheckRights))
                    {
                ...
                {
                    foreach($oExtensionInstance->EnumItems(iPopupMenuExtension::PORTAL_OBJDETAILS_ACTIONS, array('portal_id' => $combodoPortalInstanceId, 'object' => $oObject, 'mode' => $sMode)) as $oMenuItem)
                    {
			...
			else
			{
				$aPrefillFormParam = array(
					...
					'origin' => 'portal',
					'stimulus' => $requestManipulator->ReadParam('apply_stimulus', null)['code'],
                );
				$oObject->PrefillForm('state_change', $aPrefillFormParam);
			}

			// Preparing callback urls
			$aCallbackUrls = $contextManipulator->GetCallbackUrls($aActionRules, $oObject, $bModal);
			...
			{
				$sFormEndpoint = $router->generate('p_object_' . $sMode, array('sObjectClass' => $sObjectClass, 'sObjectId' => $sObjectId));
			}
			else
			{
			...
			$oFormManager->SetContainer($container)
				->SetObject($oObject)
				->SetMode($sMode)
		...
		else
		{
			...
			$sFormManagerClass = $requestManipulator->ReadParam('formmanager_class', '', FILTER_UNSAFE_RAW);
			$sFormManagerData = $requestManipulator->ReadParam('formmanager_data', '', FILTER_UNSAFE_RAW);
			if ( empty($sFormManagerClass) || empty($sFormManagerData) )
			{
				...
				throw new HttpException(500, 'Parameters formmanager_class and formmanager_data must be defined.');
			}
			...
			if (($oFormManager->GetActionRulesToken() !== null) && ($oFormManager->GetActionRulesToken() !== ''))
			{
				...
				$contextManipulator->PrepareObject($aActionRules, $oObj);
			...
			switch ($sOperation)
			{
				case 'submit':
					...
					$aFormData['validation'] = $oFormManager->OnSubmit(array('currentValues' => $requestManipulator->ReadParam('current_values', array(), FILTER_UNSAFE_RAW), 'attachmentIds' => $requestManipulator->ReadParam('attachment_ids', array(), FILTER_UNSAFE_RAW), 'formProperties' => $aFormProperties, 'applyStimulus' => $requestManipulator->ReadParam('apply_stimulus', null)));
					if ($aFormData['validation']['valid'] === true)
					{
                        ...
                        $sStimulusCode = $requestManipulator->ReadParam('stimulus_code', '');
						if (!empty($sStimulusCode))
						{
							$aFormData['validation']['redirection'] = array(
								'url' => $router->generate('p_object_apply_stimulus', array('sObjectClass' => $sObjectClass, 'sObjectId' => $oFormManager->GetObject()->GetKey(), 'sStimulusCode' => $sStimulusCode)),
								'ajax' => true
							);
						}
...
//						{
//							$aFormData['validation']['redirection'] = array(
//								'alternative_url' => $this->get('router')->generate('p_object_edit', array('sObjectClass' => $sObjectClass, 'sObjectId' => $oFormManager->GetObject()->GetKey()))
//							);
//						}
					}
					break;
				...
				case 'update':
					$oFormManager->OnUpdate(array('currentValues' => $requestManipulator->ReadParam('current_values', array(), FILTER_UNSAFE_RAW), 'formProperties' => $aFormProperties));
					break;
		...
		{
			$aRequestedFields = $requestManipulator->ReadParam('requested_fields', array(), FILTER_UNSAFE_RAW);
			$sFormPath = $requestManipulator->ReadParam('form_path', '');
	...
	public function SearchAutocompleteAction(Request $oRequest, $sTargetAttCode, $sHostObjectClass, $sHostObjectId = null)
	{
		...
		if (!isset($aRequestContent['sQuery']))
		{
			...
			throw new HttpException(500, Dict::Format('UI:Error:ParameterMissing', 'sQuery'));
		}
		...
		// Checking security layers
		if (!SecurityHelper::IsActionAllowed($scopeValidator, $isDebugEnabled, UR_ACTION_READ, $sHostObjectClass, $sHostObjectId))
		{
		...
		else
		{
			...
			// Note : The action rules must be a base64-encoded JSON object, this is just so users are tempted to changes values.
			// But it would not be a security issue as it only presets values in the form.
            $sActionRulesToken = $this->get('request_manipulator')->ReadParam('ar_token', '');
			$aActionRules = (!empty($sActionRulesToken)) ? ContextManipulatorHelper::DecodeRulesToken($sActionRulesToken) : array();
			// Preparing object
			$this->get('context_manipulator')->PrepareObject($aActionRules, $oHostObject);
		}
		...
		if (!empty($sFormManagerClass) && !empty($sFormManagerData))
		{
		    /** @var ObjectFormManager $oFormManager */
			$oFormManager = $sFormManagerClass::FromJSON($sFormManagerData);
			...
			if (($oFormManager->GetActionRulesToken() !== null) && ($oFormManager->GetActionRulesToken() !== ''))
			{
				$aActionRules = ContextManipulatorHelper::DecodeRulesToken($oFormManager->GetActionRulesToken());
				$oObj = $oFormManager->GetObject();
				/** ContextManipulatorHelper */
				$this->get('context_manipulator')->PrepareObject($aActionRules, $oObj);
				$oFormManager->SetObject($oObj);
			}
		...
		if ($oTargetAttDef->GetEditClass() !== 'CustomFields')
		{
			$oScopeSearch = $this->get('scope_validator')->GetScopeFilterForProfiles(UserRights::ListProfiles(), $sTargetObjectClass, UR_ACTION_READ);
		...
		{
			$oResponse = new Response(Dict::S('UI:ObjectDoesNotExist'), 404);
		}
	...
	public function SearchFromAttributeAction(Request $oRequest, $sTargetAttCode, $sHostObjectClass, $sHostObjectId = null)
	{
        ...
        /** @var array $combodoPortalInstanceConf */
        $combodoPortalInstanceConf = $this->getParameter('combodo.portal.instance.conf');
		...
		// Updating host object with form data / values
		$sFormManagerClass = $this->get('request_manipulator')->ReadParam('formmanager_class', '', FILTER_UNSAFE_RAW);
		$sFormManagerData = $this->get('request_manipulator')->ReadParam('formmanager_data', '', FILTER_UNSAFE_RAW);
		if ( !empty($sFormManagerClass) && !empty($sFormManagerData) )
		{
		    /** @var ObjectFormManager $oFormManager */
			$oFormManager = $sFormManagerClass::FromJSON($sFormManagerData);
			...
			// Updating host object
			$oFormManager->OnUpdate(array('currentValues' => $this->get('request_manipulator')->ReadParam('current_values', array(), FILTER_UNSAFE_RAW)));
			$oHostObject = $oFormManager->GetObject();
		}

		// Retrieving request parameters
		$iPageNumber = $this->get('request_manipulator')->ReadParam('iPageNumber', static::DEFAULT_PAGE_NUMBER, FILTER_SANITIZE_NUMBER_INT);
		$iListLength = $this->get('request_manipulator')->ReadParam('iListLength', static::DEFAULT_LIST_LENGTH, FILTER_SANITIZE_NUMBER_INT);
		$bInitalPass = $this->get('request_manipulator')->HasParam('draw') ? false : true;
		$sQuery = $this->get('request_manipulator')->ReadParam('sSearchValue', '');
		$sFormPath = $this->get('request_manipulator')->ReadParam('sFormPath', '');
		$sFieldId = $this->get('request_manipulator')->ReadParam('sFieldId', '');
		$aObjectIdsToIgnore = $this->get('request_manipulator')->ReadParam('aObjectIdsToIgnore', null, FILTER_UNSAFE_RAW);
		...
		$aAttCodes = ApplicationHelper::GetLoadedListFromClass($combodoPortalInstanceConf, $sTargetObjectClass, 'list');
		...
		while ($oItem = $oSet->Fetch())
		{
			$aItems[] = $this->PrepareObjectInformations($oItem, $aAttCodes);
		}
     ...
     *
     */
	public function SearchHierarchyAction(Request $oRequest, $sTargetAttCode, $sHostObjectClass, $sHostObjectId = null)
	{
		// TODO
	}
	...
	public function DocumentAction(Request $oRequest, $sOperation = null)
    {
        ...
        $sObjectClass = $this->get('request_manipulator')->ReadParam('sObjectClass', '');
        $sObjectId = $this->get('request_manipulator')->ReadParam('sObjectId', '');
        $sObjectField = $this->get('request_manipulator')->ReadParam('sObjectField', '');
        ...
        if (($sHostId > 0) && !SecurityHelper::IsActionAllowed($scopeValidator, $isDebugEnabled, UR_ACTION_READ, $sHostClass, $sHostId))
        {
        ...
        // Retrieving object
        $oObject = MetaModel::GetObject($sObjectClass, $sObjectId, false /* Must not be found */, $this->get('scope_validator')->IsAllDataAllowedForScope(UserRights::ListProfiles(), $sHostClass));
        if ($oObject === null)
        {
            // We should never be there as the secuirty helper makes sure that the object exists, but just in case.
            IssueLog::Info(__METHOD__ . ' at line ' . __LINE__ . ' : Could not load object ' . $sObjectClass . '::' . $sObjectId . '.');
        ...
        {
            $iCacheSec = $this->get('request_manipulator')->ReadParam('cache', 0, FILTER_SANITIZE_NUMBER_INT);
        }
	...
	public function AttachmentAction(Request $oRequest, $sOperation = null)
	{
		...
		if ($sOperation === null)
		{
			$sOperation = $this->get('request_manipulator')->ReadParam('operation', null);
		}
		switch ($sOperation)
		{
			case 'add':
				$sFieldName = $this->get('request_manipulator')->ReadParam('field_name', '');
				$sObjectClass = $this->get('request_manipulator')->ReadParam('object_class', '');
				$sTempId = $this->get('request_manipulator')->ReadParam('temp_id', '');
				...
				$oResponse = new JsonResponse($aData, 200, array('Content-Type' => 'text/plain'));
				break;
...
//                    'sObjectId' => $this->get('request_manipulator')->ReadParam('sAttachmentId', null),
//                    'sObjectField' => 'contents',
//                );
//                $sRedirectRoute = $this->get('router')->generate('p_object_document_download', $aRouteParams);
//                // - Request
//                $oSubRequest = Request::create($sRedirectRoute, 'GET', $oRequest->query->all(), $oRequest->cookies->all(), array(), $oRequest->server->all());
//
//                $oResponse = $oApp->handle($oSubRequest, HttpKernelInterface::SUB_REQUEST, true);
	...
	public function GetInformationsAsJsonAction(Request $oRequest)
	{
		...
		// Retrieving parameters
		$sObjectClass = $this->get('request_manipulator')->ReadParam('sObjectClass', '');
		$aObjectIds = $this->get('request_manipulator')->ReadParam('aObjectIds', array(), FILTER_UNSAFE_RAW);
		$aObjectAttCodes = $this->get('request_manipulator')->ReadParam('aObjectAttCodes', array(), FILTER_UNSAFE_RAW);
		if ( empty($sObjectClass) || empty($aObjectIds) || empty($aObjectAttCodes) )
		{
			...
			throw new HttpException(500, 'Invalid request data, some informations are missing');
		}
		...
		// Building the search
		$bIgnoreSilos = $this->get('scope_validator')->IsAllDataAllowedForScope(UserRights::ListProfiles(), $sObjectClass);
		...
		{
			$aData['items'][] = $this->PrepareObjectInformations($oObject, $aObjectAttCodes);
		}
     ...
     * @throws \OQLException
     */
	protected function PrepareObjectInformations(DBObject $oObject, $aAttCodes = array())
	{
		...
		foreach ($aAttDefs as $oAttDef)
		{
			...
			if ($oAttDef->IsExternalKey())
			{
				...
				if (SecurityHelper::IsActionAllowed($scopeValidator, $isDebugEnabled, UR_ACTION_READ, $oAttDef->GetTargetClass()))
				{
					$aAttData['url'] = $this->get('router')->generate('p_object_view', array('sObjectClass' => $oAttDef->GetTargetClass(), 'sObjectId' => $oObject->Get($oAttDef->GetCode())));
				}
			}
			...
			elseif ($oAttDef instanceof AttributeImage)
            {
                ...
                {
                    $sUrl = $this->get('router')->generate('p_object_document_display', array('sObjectClass' => get_class($oObject), 'sObjectId' => $oObject->GetKey(), 'sObjectField' => $oAttDef->GetCode(), 'cache' => 86400));
                }
                else
                {
			...
			else
			{
				...
				if ($oAttDef instanceof AttributeFriendlyName)
				{
					...
					if(SecurityHelper::IsActionAllowed($scopeValidator, $isDebugEnabled, UR_ACTION_READ, $sObjectClass))
					{
						$aAttData['url'] = $this->get('router')->generate('p_object_view', array('sObjectClass' => $sObjectClass, 'sObjectId' => $oObject->GetKey()));
					}
				}
			}

...
class ManageBrickController extends BrickController
{
    ...
    public function DisplayAction(Request $oRequest, $sBrickId, $sGroupingTab, $sDisplayMode = null)
    {
        ...
        return $this->render($sLayoutTemplate, $aData);
    }
    ...
    public function ExcelExportStartAction(
        Request $oRequest, $sBrickId, $sGroupingTab, $sGroupingArea
    ) {
        /** @var BricksCollection  $bricksCollection */
        $bricksCollection = $this->get('bricks_collection');
        ...
        else
        {
            ...
            $oScopeHelper = $this->get('scope_validator');
    ...
    {
        /** @var \Combodo\iTop\Portal\Brick\ManageBrick $oBrick */
        $oBrick =     $this->get('bricks_collection')->getBrickById($sBrickId);
        ...
        $sDataLoading = $this->get('request_manipulator')->ReadParam('sDataLoading', $oBrick->GetDataLoading());

        // - Retrieving the grouping areas to display
        $sGroupingArea = $this->get('request_manipulator')->ReadParam('sGroupingArea', '');
        ...
        if ($bNeedDetails)
        {
            ...
            if ($sDataLoading === AbstractBrick::ENUM_DATA_LOADING_AUTO)
            {
                ...
                $fThreshold = (float)MetaModel::GetModuleSetting(
                    $this->getParameter('combodo.portal.instance.id'),
            ...
            foreach ($aQueries as $sKey => $oQuery)
            {
                ...
                if ($oQuery !== null)
                {
                    ...
                    if ($sDataLoading === AbstractBrick::ENUM_DATA_LOADING_LAZY)
                    {
                        ...
                        $iListLength = (int)$this->get('request_manipulator')->ReadParam('iListLength', ManageBrick::DEFAULT_LIST_LENGTH, FILTER_SANITIZE_NUMBER_INT);
                    ...
                    SecurityHelper::PreloadForCache(
                        $this->get('scope_validator'),
                        $oSet->GetFilter(),
                ...
                while ($oCurrentRow = $oSet->Fetch())
                {
                    ...
                    foreach ($aColumnsAttrs as $sItemAttr)
                    {
                        ...
                        if ($sItemAttr === $sMainActionAttrCode)
                        {
                            ...
                            if (($oBrick->GetOpeningMode() === ManageBrick::ENUM_ACTION_EDIT) && SecurityHelper::IsActionAllowed(
                                $this->get('scope_validator'),
                            ...
                            elseif (SecurityHelper::IsActionAllowed(
                                $this->get('scope_validator'),
                                $this->get('kernel')->isDebug(),
                                UR_ACTION_READ,
                                $sCurrentClass,
                        ...
                        if ($oAttDef->IsExternalKey())
                        {
                            ...
                            if ($oCurrentRow->Get($sItemAttr) !== $oAttDef->GetNullValue())
                            {
                                ...
                                if (SecurityHelper::IsActionAllowed(
                                    $this->get('scope_validator'),
                                    $this->get('kernel')->isDebug(),
                                    UR_ACTION_READ,
                        ...
                        elseif ($oAttDef instanceof AttributeImage)
                        {
                            ...
                            {
                                $sUrl = $this->get('router')->generate('p_object_document_display', array('sObjectClass' => get_class($oCurrentRow), 'sObjectId' => $oCurrentRow->GetKey(), 'sObjectField' => $sItemAttr, 'cache' => 86400));
                            }
                            else
                            {
                    ...
                    {
                        foreach (
                            $oExtensionInstance->EnumItems(
                                iPopupMenuExtension::PORTAL_OBJLISTITEM_ACTIONS,
                                array(
                                    'portal_id' => $this->getParameter('combodo.portal.instance.id'),
                                    'object' => $oCurrentRow
                                )
                            )
                            as $oMenuItem
        ...
        else
        {
            ...
            if ($bHasScope)
            {
                foreach ($aGroupingTabsValues as $aValues)
                {
                    ...
                    $aUrls[] = $this->get('router')->generate(
                        'p_manage_brick',
                        array(
                            'sBrickId' => $sBrickId,
